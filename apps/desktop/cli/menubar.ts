import { chmodSync, copyFileSync, mkdirSync, readFileSync, renameSync, rmSync, lstatSync, writeFileSync } from "node:fs";
...
function exists(path: string): boolean { try { const info = lstatSync(path); return info.isFile() && (info.mode & 0o111) !== 0 && (info.mode & 0o022) === 0; } catch { return false; } }
