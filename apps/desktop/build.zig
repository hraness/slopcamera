...
pub fn build(b: *std.Build) void {
    ...
    if (selected_platform == .macos and target.result.cpu.arch != .aarch64) {
        @panic("Slopcamera currently packages pinned Apple Silicon runtimes and requires a macOS arm64 target");
    }
    ...
    options.addOption([]const u8, "minimum_macos_version", minimum_macos_version);
    ...
    const frontend_build = b.addSystemCommand(&.{ "bun", "run", "--cwd", "../..", "build:desktop:frontend" });
    ...
    const dev = b.addSystemCommand(&.{ "bunx", "--no-install", "native", "dev", "--manifest", "app.zon", "--binary" });
    ...
    const package = b.addSystemCommand(&.{
        ...
        "--output",
        b.fmt("zig-out/package/{s}-3.2.7-{s}-{s}{s}", .{ app_exe_name, @tagName(package_target), package_optimize_name, packageSuffix(package_target) }),
        "--binary",
    });
    ...
    if (platform == .macos) {
        switch (web_engine) {
            .system => {
                ...
                const flags: []const []const u8 = if (b.sysroot) |sysroot| &.{ "-fobjc-arc", "-fno-sanitize=builtin", "-ObjC", minimum_macos_flag, "-isysroot", sysroot, sdk_include } else &.{ "-fobjc-arc", "-fno-sanitize=builtin", "-ObjC", minimum_macos_flag };
            ...
            .chromium => {
                ...
                const flags: []const []const u8 = if (b.sysroot) |sysroot| &.{ "-fobjc-arc", "-fno-sanitize=builtin", "-ObjC++", "-std=c++17", "-stdlib=libc++", minimum_macos_flag, "-isysroot", sysroot, sdk_include, include_arg, define_arg } else &.{ "-fobjc-arc", "-fno-sanitize=builtin", "-ObjC++", "-std=c++17", "-stdlib=libc++", minimum_macos_flag, include_arg, define_arg };
