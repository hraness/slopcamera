...
The portable projection contains four operations: diagram check, diagram render, image generate, and image vectorize. The complete host adds analysis, derive, edit, gateway, iteration, media, project, render, scene, slopcamera, and spatial operation families — 67 registered kinds in this source, enumerable without probing the runtime through `slopcamera operations list --json` and `slopcamera capabilities --json`. The `scene` family includes the direction, effects, and temporal-audit planning operations a cinematic graph composes. A graph that names an unsupported operation fails at compilation, before executor or resource admission. An operation input is typed data, never a caller-selected executable, shell command, dynamic loader, or registration hook. The [SDK reference](/docs/reference/sdk) records each entrypoint's scope and execution effects.
...
- Spatial recipe packs are inert, content-addressed JSON documents (`slopcamera.spatial-recipe-pack`) that an agent authors to parameterize the cinematic-world workflow: one scene identity, one direction document, bounded gallery axes, preview render requests, optional declared effects, and an optional temporal audit. A pack cannot name executors, source paths, permissions, secrets, or runtime URLs; it only selects and bounds what the closed registry already allows.
...
`slopcamera mcp --root /absolute/workspace` serves a fixed 15-tool set to MCP clients. The `check_diagram` and `render_diagram` compatibility tools sit beside `search_slopcamera` and `execute_slopcamera` over the bounded portable registry, plus root-relative scene tools: `check_scene`, `inspect_scene`, `diff_scenes`, `evaluate_scene`, `audit_scene`, `audit_scene_temporal`, `check_scene_direction`, `plan_scene_direction`, `plan_scene_gallery`, `check_scene_effects`, and `plan_scene_effects`. Paths are root-relative to the selected root, a checked diagram admits at most 64 shapes and 128 edges with at most 40 reported findings, and `execute_slopcamera` accepts only the six registered operation codes and never evaluates source code. The scene tools are read-mostly: they plan and audit scene, direction, and effects documents without mutating project state, so selection and promotion stay in explicit CLI or SDK calls. The diagram tools do not load workspace configuration. MCP stays a deliberate subset: a client that needs project editing, native jobs, or promotion uses the CLI or SDK. The [MCP tutorial](/docs/tutorials/mcp) covers client setup.
