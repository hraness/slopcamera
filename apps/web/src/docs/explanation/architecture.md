...
The portable SDK owns the canonical contracts: diagram schemas, declarative graphs, spatial-scene documents, and native-job definitions. The complete local host consumes those contracts through its own closed projection and adds durable jobs, media import, rendering, project storage, and resource admission. The `slopcamera` CLI owns that host interface; an optional unbundled menu-bar companion only presents agent outputs and has no capture, editing, or project authority. `slopcamera mcp` exposes a deliberately smaller boundary to other clients: compatibility diagram tools and the bounded portable registry.
...
Project state lives locally. Slopcamera has no product account and no hosted project database; the local CLI host, the portable SDK, and the MCP boundary all operate on files and receipts on your machine. Local rendering consumes admitted assets, while explicit acquisition and provider operations cross separate network boundaries: Gateway sends authorized prompts and named references to models, hosted image generation sends one prompt to `api.slopcamera.com` against prepaid Hraness Credits, private Blob hosting grants temporary reference access, Poly Haven imports selected assets, and first-use provisioning may download verified tool dependencies.
