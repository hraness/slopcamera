...
This guide is the durable-graph counterpart of the scene, direction, and effects commands covered in [Render and edit spatial scenes](/docs/how-to/direct-scenes). The commands ship in current source; check `slopcamera help scene` and `slopcamera workflows show cinematic-world --json` for the installed contract before relying on them in a release.
