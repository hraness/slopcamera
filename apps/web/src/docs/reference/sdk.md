...
The portable projection contains four operations: `slopcamera.diagram.check`, `slopcamera.diagram.render`, `slopcamera.image.vectorize`, and `slopcamera.image.generate`. The remaining operation codes, `slopcamera.image.icon` and `slopcamera.image.gallery`, run only through `execute_slopcamera`, `executeSlopcameraOperation`, and the CLI, because each runs several paid Gateway calls and can publish more than one artifact. Portable spatial and studio schemas can parse, hash, and plan values without making their local executors available.
...
The MCP server exposes 17 named tools: `check_diagram`, `render_diagram`, `search_slopcamera`, `execute_slopcamera`, and 13 scene tools. These include scene inspection and evaluation, direction and gallery planning, effects planning, temporal audit, and behavior check/audit. The operation registry behind `execute_slopcamera` has six operation codes; the typed portable projection covers four of them. Paths are root-relative and configuration is inert. Diagram tools admit at most 64 shapes and 128 edges with at most 40 reported findings. Generated `.tldr` output is editable interchange; diagram JSON remains the authored source. See [the MCP setup page](/docs/tutorials/mcp) for the released tool tables and setup.