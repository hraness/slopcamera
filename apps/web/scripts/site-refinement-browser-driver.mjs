...
async function main() {
  ...
  const request = parseRefinementRequest(decodeWorkerJson(await readPreviewFile(requestPath, workerProtocolLimit)))
  const copy = request.scope === refinementCopyScope
  const selectedCases = refinementCases(request.scope)
  const selectedDeadline = refinementDeadline(request.scope)
  const parsePhase = parseRefinementPhase
  ...
  const result = await withPreviewCancellation(process, async cancellation => {
      ...
      try {
        ...
        const [evidence, old] = await cancellation.wait(() => {
          ...
          activePair = copy ? settleShellPair(
            () => checkCopyCase(browser, request.current, scenario, negative, refinementCopyProfile, async (page, state, elements) => {
              if (state === "idle-focus") await refinementDom(page, true, scenario)
              await observeRefinementCopyPaint(page, scenario, state, elements)
            }),
        ...
        stage = "comparison"
        if (copy) {
          observations.push(compareRefinementCopy(evidence, old, scenario, negative))
        }
        else {
        ...
        try {
          await bounded(writeFile(join(dirname(requestPath), "site-marketing-case-failure.json"),
            encodeWorkerJson(refinementCaseFailure(request, scenario.name, stage, cases, error)), { flag: "wx", mode: 0o600 }),
    ...
    return { ...common, ...runtime, sequence: 2, kind: "result", browser: browser.version(),
      cases, comparison: copy ? "refined-copy-with-original-state-machine" : "unchanged-shell-with-closed-refinement-islands", negativeControls, closed: true, observations }
  }, async () => {
    const failures = []
