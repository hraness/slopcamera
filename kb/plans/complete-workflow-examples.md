...
The source registry contains 40 reviewed examples and six selected homepage examples. The original 32 are joined by an automatic diagram stack, its custom-font/icon treatment, a native straight-alpha PNG / premultiplied linear EXR chart, a 72-frame focus study, and four separate camera films: dolly, crane, rail and tripod. These are prepared for publication; production has not been updated by this task. The last completed source admission verified 104 distinct retained files for 36 examples; source, rendered-anchor and link checks for all 40 are queued.

The original 123 technique observations remain fixed: 36 demonstrated, 10 partial, 34 planned, 20 source-checked, 11 budget-needed and 12 input-needed. Five separately identified parametric witnesses are also demonstrated, making 41 of 128 completed observations and 87 unfinished. A media record is not a completed technique by itself. The color chart passed 26 numeric conditions and independent edge/composite review; its public PNG preserves exact decoded pixels and color chunks after removing private metadata. The diagram pair retains ten real exports and editable graph evidence. The focus study has closed native custody and all 72 frames reviewed. Each new camera film has 192 reviewed frames, exact source/output hashes and calibrated camera samples.

The earlier full website suite passed 520 tests and 47,297 assertions. The final 133-case gate (76 shell, eight copy, 40 docs and nine player cases) and literal-island admission remain pending. The mobile documentation disclosure has native keyboard/no-JavaScript observations in a finite14-case diagnostic queued with the sole browser owner. Earlier diagnostics confirmed native playback, portrait containment, denied-media recovery and no unexpected requests. A measured docs grid repair reduced 390px viewport scrollWidth from 724 to 390. The missing docs favicon is fixed with a built-resource regression. Historical profiles retain their contracts.
...
Delivered main 437a530 and verified v3.3.1 are integrated. The independently rebuilt native baseline uses exact revision 437a530ee81bd0816c911f216f4331c88770bf31, tree 4a453ddc4aecd6c68a2135dcf447dc323efa6c6d, and a schema-six manifest. Release parity covers 17 MCP tools, six portable operation codes and eight built-in workflows. Each earlier example retains its actual execution version and source identity. Later renderer corrections need the next verified release.

Inactive hosted request/state code passes 18 deterministic tests with 2,787 assertions, focused TypeScript and lint. Independent review corrected terminal-billing evidence retention and verifies dispatch timing bounds. It has no service endpoint, public adapter, payment or provider call. Existing Credits registration does not prove an inference backend. Commercial choices, owning-service pickup recovery, live qualification and deployment remain open. Required final integration, isolated web installation, native browser, release and production gates still apply.
