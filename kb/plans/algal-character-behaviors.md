...
## Upstream ALGAL dogfood findings

Friction discovered by Slopcamera's behavior integration — each feeds back to `hraness/algal` as a concrete consumer report.

1. **Round-summary receipts** — `repeat` cell records scale linearly with rounds (18 records over 8 ticks). Dense-tick loops accumulate large receipts; a round-summary or scan/fold primitive would compress receipt output while preserving verification. Mitigation: chunk ticks inside fns (one `repeat` round simulates K ticks via spec data), bounded `maxRounds`.
2. **No `repeat`-native iteration input** — sequential automata need carried windows because `repeat` feeds identical inputs each round. A native `iterate` input binding that threads the index or window position would eliminate the `window.advance.v1` shim kernel. Mitigation: `window.advance.v1` + carry `{rest→win}`.
3. **`until.equals` is string-only** — numeric termination conditions (e.g., "done when counter reaches N") require text sentinel ports because `until` compares output text. Mitigation: `window.advance.v1` outputs `done` as text `"true"`/`"false"`.
4. **`each` has no carry** — per-item state threading is impossible; `each` is pure fan-out with no inter-iteration state. Sequential operations over entity lists require an external orchestration pattern. Mitigation: use `repeat`+carry for sequential iteration instead of `each`.
5. **`noUnusedParameters` gap** — algal's own tsconfig lacks the flag, so `graph.ts:interfaceSignature` shipped a dead `cellId` parameter. Strict downstream consumers (Slopcamera) catch it. Fixed upstream: `f6991d2`.
6. **`many: true` composition gap** — `behavior.combine.v1`'s `layers` port is single-edge, and `many: true` isn't part of the fn signature contract. Composing N sub-organism traces requires N-1 pairwise `emitted.append.v1` chains rather than a natural multi-edge merge. A `many`-aware fn port convention would simplify composition.
...
- 2026-09-20 — Phase F implemented. `src/spatial-scene/behavior-authoring.test.ts`: end-to-end authoring proof — three tests composing stdlib organisms into a scene-bound behavior doc, running the full check→bake→audit→gallery pipeline. Full pipeline test verifies: check finds no errors, bake produces emitted records on all declared channels (locomotion, face.blink, interact, prop.attach), audit reports no errors (advisory unreachable-state warning for "idle" is expected since the always guard fires before first emission), gallery produces distinct seeded candidates. Incremental authoring test: agent starts with locomotion-only, upgrades to the combined organism, both pass check+bake. Deterministic replay test: identical doc+seed+scene produce byte-identical receipt digests and bake hashes. Upstream ALGAL dogfood findings documented in the plan: round-summary receipts, no repeat-native iteration, string-only until.equals, each without carry, noUnusedParameters gap (fixed f6991d2), many:true composition gap. Agent skill reference `directed-scenes.md` updated with "Author character behaviors" section covering the behavior document schema, four CLI commands (check/bake/gallery/audit), stdlib organisms, the authoring loop pattern, and the seed/channel-map conventions. AGENTS.md updated. 154 tests pass across 12 files, tsc×2 clean.
