...
The reusable checklist per product: pick the portable capability subset, add the transport adapter (REST + MCP + OpenAPI), register every new storage/provider surface in `costs.json`, and produce the platform evidence pack — example prompts, privacy and support links that match actual behavior, and terms — before submitting anywhere. Slopcamera's pack lives in `docs/platform-submission.md` with verified live calls and per-platform readiness.

Two serverless adaptations proved necessary on Vercel and will recur on other hosts: machine-global host-resource admission has no flock state in a function sandbox, so the runtime injects a process-local coordinator that still bounds claims; and the detected framework preset degenerates catch-all function routes, so the deployable surface is one thin file per endpoint rather than a single catch-all.
