# Render motion graphics from HTML

Use `slopcamera html render` to turn authored HTML, SVG, Canvas, shaders, or Three.js into a video while retaining the document, assets, render request, and an editable project. All seven HTML profiles ship in v3.2.8; each needs its admitted local browser and FFmpeg/FFprobe. The source files below were authored later and live in the current repository.

For a first guided render, use [Create and revise your first animation](../tutorials/first-animation.md). This guide assumes an installed CLI, a chosen graphic, and a current repository checkout containing `examples/showcase/html/`. Run commands from that checkout's root so resource paths resolve correctly.
...
To revise copy, palette, or timing, change parameters supported by the document or edit the HTML, then render a new request. [editorial-revised.json](https://github.com/hraness/slopcamera/blob/main/examples/showcase/html/editorial-revised.json) changes copy and accent while retaining the original source. Keep the selected result's `projectId` for [editing and delivery](../how-to/edit-video.md).
...
[Watch the example](https://slopcamera.com/docs/how-to/render-motion-graphics#draw-procedural-contours).
...
[Watch the example](https://slopcamera.com/docs/how-to/render-motion-graphics#choreograph-vector-shapes).
...
[Watch the example](https://slopcamera.com/docs/how-to/render-motion-graphics#shape-a-shader-background).
...
[Watch the example](https://slopcamera.com/docs/how-to/render-motion-graphics#write-a-wgsl-field).
...
[Watch the example](https://slopcamera.com/docs/how-to/render-motion-graphics#compose-a-transparent-lower-third).

The title enters, holds and leaves over a colored preview plate. The HTML source also produces real transparent pixels when used through `workflow.media.htmlOverlay`. The published MP4 is opaque; check the transparent derivative over your destination footage before delivery.
