...
Install [Slopcamera](../../README.md#install-slopcamera) on macOS or Linux. Windows deliberately rejects this vectorization profile. First use may download a checksum-pinned VTracer archive; subsequent tracing uses the prepared local tool. Begin with an image you have permission to use and an output path you intend to write.
...
To reproduce the pair, use a [source checkout](use-current-source.md) with dependencies installed and run from its root:
