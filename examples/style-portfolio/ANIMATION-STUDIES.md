...
The composition is 1920 × 1080 and scales to the physical canvas while preserving its aspect ratio. The viewport and device scale determine the raster size. Physical canvases are bounded to 8192 pixels per side and 33,554,432 pixels total. A 3840 × 2160 render redraws the geometry at that resolution. Pixel art is intentionally drawn on a 320 × 180 grid, enlarged by the largest fitting integer scale with nearest sampling. Letterboxing preserves the composition in a different viewport shape. Canvas edge rasterization and video encoding can introduce additional colors; this is not a strict indexed-palette export.
