...
// src/cloud-errors.ts
class SlopcameraCloudError extends Error {
  code;
...
// src/generate.ts
var slopcameraGatewayApiBaseUrl = "https://ai-gateway.vercel.sh/v4/ai";
var slopcameraImageModels = Object.freeze([
  "openai/gpt-image-1.5",
  "recraft/recraft-v4.1-utility"
]);
var slopcameraResponseMediaTypes = Object.freeze([
  "image/png",
  "image/jpeg",
  "image/webp"
]);
var slopcameraMaximumPromptBytes = 32 * 1024;
var slopcameraMaximumRawImageBytes = 64 * 1024 * 1024;
...
function credentialValue(value) {
  if (value.length < 16 || value.length > 16 * 1024 || value.trim() !== value || /[^\x21-\x7e]/u.test(value)) {
    throw new SlopcameraCloudError("AUTHENTICATION_REQUIRED", "The selected Vercel AI Gateway credential is invalid.");
  }
  return value;
}
...
function resolveSlopcameraGatewayCredential(injected) {
  ...
  throw new SlopcameraCloudError("AUTHENTICATION_REQUIRED", "Set AI_GATEWAY_API_KEY or run Slopcamera through `vercel env run -- \u2026` with VERCEL_OIDC_TOKEN available.");
}
function slopcameraGatewayCredentialStatus(injected) {
  const values = environment(injected);
  if (values.AI_GATEWAY_API_KEY !== undefined) {
...
function assertGenerationActive(signal) {
  if (signal.aborted) {
    throw new SlopcameraCloudError("GENERATION_FAILED", "Vercel AI Gateway image generation was cancelled or exceeded its deadline.");
  }
}
...
function gatewayUrl(input) {
  try {
    ...
    throw new SlopcameraCloudError("GENERATION_FAILED", "The Vercel AI Gateway request was rejected.");
  }
}
...
function boundedResponse(response, maximumBytes) {
  ...
  if (declared !== null) {
    ...
    if (!Number.isSafeInteger(value) || value < 0 || value > maximumBytes) {
      ...
      throw new SlopcameraCloudError("GENERATION_INVALID_RESPONSE", "Vercel AI Gateway returned an invalid bounded response.");
    }
  }
    ...
    pull: async (controller) => {
      try {
        ...
        if (bytes > maximumBytes) {
          ...
          controller.error(new SlopcameraCloudError("GENERATION_INVALID_RESPONSE", "Vercel AI Gateway returned an invalid bounded response."));
          return;
        }
...
function combineSignals(caller, timeoutMs) {
  ...
  const rejectOnAbort = () => {
    rejectInterruption?.(new SlopcameraCloudError("GENERATION_FAILED", "Vercel AI Gateway image generation was cancelled or exceeded its deadline."));
  };
...
function mediaType(value) {
  if (typeof value !== "string" || !slopcameraResponseMediaTypes.includes(value)) {
    throw new SlopcameraCloudError("GENERATION_INVALID_RESPONSE", "Vercel AI Gateway returned an unsupported image type.");
  }
  return value;
}
...
function parseResult(value, model) {
  if (!isObject(value) || !Array.isArray(value.images) || value.images.length !== 1) {
    throw new SlopcameraCloudError("GENERATION_INVALID_RESPONSE", "Vercel AI Gateway did not return exactly one image.");
  }
...
async function atomicImageWrite(outputPath, bytes) {
  ...
  try {
    ...
    throw new SlopcameraCloudError("OUTPUT_WRITE_FAILED", "Slopcamera could not atomically write the generated image.");
  } finally {
