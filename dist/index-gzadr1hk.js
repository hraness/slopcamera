...
import { z } from "zod";
var WORKFLOW_GRAPH_VERSION = "slopcamera-workflow-graph-v2";
var WORKFLOW_REF_VERSION = "slopcamera-workflow-ref-v1";
var GRAPH_ABI = "slopcamera-workflow-graph-abi-v2";
var REQUIREMENT_ENVELOPE_VERSION = "slopcamera-requirement-envelope-v2";
var TRUSTED_COMPUTE_VERSION = 1;
var WORKFLOW_COMPILATION_VERSION = "slopcamera-workflow-compilation-v1";
...
var TRUSTED_COMPUTE_BRAND = Symbol.for("slopcamera.trusted-compute-definition");
...
function parseCodeBoundary(schema, input, name) {
  ...
  if (!result.success) {
    throw new SlopcameraCodeError("invalid-data", `Invalid ${name}.`, {
...
var WORKFLOW_REGISTRY_PROJECTION_HASH_DOMAIN = "slopcamera.workflow.registry-projection/v1";
var PUBLIC_WORKFLOW_REGISTRY_PROJECTION_ID = "slopcamera.workflow.registry.public/v1";
...
function boundedOperationDiscoveryList(input, name = "operation discovery list") {
  ...
  if (length > MAX_OPERATION_DISCOVERY_ENTRIES) {
    throw new SlopcameraCodeError("invalid-data", `${name} cannot exceed ${String(MAX_OPERATION_DISCOVERY_ENTRIES)} entries.`);
  }
  const descriptors = Object.getOwnPropertyDescriptors(input);
  if (Object.getOwnPropertySymbols(descriptors).some((symbol) => Reflect.get(descriptors, symbol)?.enumerable === true)) {
    throw new SlopcameraCodeError("invalid-data", `${name} cannot contain enumerable symbol properties.`);
  }
  ...
  if (keys.length !== length || keys.some((key, index) => key !== String(index))) {
    throw new SlopcameraCodeError("invalid-data", `${name} must be dense and cannot have named properties.`);
  }
  ...
  for (let index = 0;index < length; index += 1) {
    ...
    if (descriptor === undefined || descriptor.get !== undefined || descriptor.set !== undefined) {
      throw new SlopcameraCodeError("invalid-data", `${name} must contain plain data elements.`);
    }
    const remainingBytes = MAX_OPERATION_DISCOVERY_LIST_BYTES - bytes;
    if (remainingBytes < 1) {
      throw new SlopcameraCodeError("invalid-data", `${name} contains more than ${String(MAX_OPERATION_DISCOVERY_LIST_BYTES)} bytes.`);
    }
    ...
    if (values > MAX_OPERATION_DISCOVERY_LIST_VALUES) {
      throw new SlopcameraCodeError("invalid-data", `${name} contains more than ${String(MAX_OPERATION_DISCOVERY_LIST_VALUES)} JSON values.`);
    }
...
function canonicalSlopcameraIdentity(value) {
  if (value === "studio")
    return "slopcamera";
...
function normalizeOperationDiscoveryPreservingIdentity(input) {
  const normalized = boundedOperationDiscoveryList(input).map((item) => {
    ...
    if (preparation.length !== parsed.policy.preparation.length) {
      throw new SlopcameraCodeError("invalid-data", `Duplicate preparation requirement for ${operationKey(parsed.kind, parsed.version)}.`, { kind: parsed.kind, version: parsed.version });
    }
    ...
    if (new Set(resources.map((resource) => resource.resource)).size !== resources.length) {
      throw new SlopcameraCodeError("invalid-data", `Duplicate resource claim for ${operationKey(parsed.kind, parsed.version)}.`, { kind: parsed.kind, version: parsed.version });
    }
  ...
  for (const operation of normalized) {
    ...
    if (seen.has(key)) {
      throw new SlopcameraCodeError("conflict", `Duplicate operation discovery entry: ${key}`, { kind: operation.kind, version: operation.version });
    }
    seen.add(key);
  }
  return normalized;
...
function createWorkflowRegistryProjectionHash(input) {
  const id = canonicalSlopcameraIdentity(parseCodeBoundary(SchemaIdSchema, input.id, "registry projection id"));
...
function createWorkflowRegistryProjection(idInput, source, options = {}) {
  const id = canonicalSlopcameraIdentity(parseCodeBoundary(SchemaIdSchema, idInput, "registry projection id"));
...
function parseWorkflowRegistryProjection(input) {
  ...
  if (parsed.projectionSha256 !== expectedProjectionSha256) {
    throw new SlopcameraCodeError("invalid-data", "Workflow registry projection hash does not match its contents.", {
  ...
  if (canonicalJsonSha256(parsed.discovery) !== canonicalJsonSha256(exactDiscovery)) {
    throw new SlopcameraCodeError("invalid-data", "Workflow registry projection discovery is not normalized.", { projectionId: parsed.id });
  }
...
function canonicalSlopcameraIdentity2(value) {
  if (value === "studio")
    return "slopcamera";
  return value.replace(/^studio\./u, "slopcamera.");
}
...
function canonicalizeGraphValue(value) {
  if (isSerializedRef(value)) {
    return {
      $ref: {
        ...value.$ref,
        schemaId: canonicalSlopcameraIdentity2(value.$ref.schemaId)
      },
    ...
    nodes: graph.nodes.map((node) => ({
      ...
      executor: node.executor.kind === "operation" ? {
        ...
        operation: {
          ...
          kind: canonicalSlopcameraIdentity2(node.executor.operation.kind)
        }
      } : {
        compute: {
          ...node.executor.compute,
          key: canonicalSlopcameraIdentity2(node.executor.compute.key)
        },
        kind: "compute"
      },
      input: canonicalizeGraphValue(node.input),
      inputSchemaId: canonicalSlopcameraIdentity2(node.inputSchemaId),
      outputSchemaId: canonicalSlopcameraIdentity2(node.outputSchemaId)
    })),
    outputs: canonicalizeGraphValue(graph.outputs),
    version: WORKFLOW_GRAPH_VERSION,
    workflow: {
      ...graph.workflow,
      id: canonicalSlopcameraIdentity2(graph.workflow.id),
      inputSchemaId: canonicalSlopcameraIdentity2(graph.workflow.inputSchemaId)
    }
  ...
  for (const node of graph.nodes) {
    ...
    if (isOperationGraphNode(node)) {
      ...
      if (operation === undefined) {
        throw new SlopcameraCodeError("unsupported-plan", `Unsupported operation: ${operationKey2(identity.kind, identity.version)}`, {
          kind: identity.kind,
          nodeKey: node.key,
      ...
      if (!projection.trustedCompute) {
        throw new SlopcameraCodeError("unsupported-plan", `Trusted compute is unsupported at node ${node.key}.`, {
      ...
      throw new SlopcameraCodeError("internal", `Unknown node executor for ${node.key}.`);
    }
...
function operationFamily(kind) {
  ...
  if (separator < 1) {
    throw new SlopcameraCodeError("internal", `Operation ${kind} has no namespace family.`, { kind });
  }
...
function deriveRequirementEnvelope(validated) {
  ...
  for (const node of validated.graph.nodes) {
    ...
    if (policy === undefined) {
      throw new SlopcameraCodeError("internal", `Node ${node.key} lost its execution policy.`);
    }
...
function canonicalizeRequirementEnvelope(input) {
  ...
  return parseCodeBoundary(RequirementEnvelopeSchema, {
    ...envelope,
    computeKeys: envelope.computeKeys.map(canonicalSlopcameraIdentity2),
    operationFamilies: envelope.operationFamilies.map(canonicalSlopcameraIdentity2),
    operationKinds: envelope.operationKinds.map(canonicalSlopcameraIdentity2),
    version: REQUIREMENT_ENVELOPE_VERSION
...
function resolveProjection(options) {
  if (options.projection !== undefined) {
    if (options.registry !== undefined || options.projectionId !== undefined || options.trustedCompute !== undefined) {
      throw new SlopcameraCodeError("invalid-data", "Compile with either a projection or a registry projection source, not both.");
    }
    return parseWorkflowRegistryProjection(options.projection);
  }
  if (options.registry !== undefined) {
    if (options.projectionId === undefined) {
      throw new SlopcameraCodeError("invalid-data", "A registry projection source requires an explicit projection id.");
    }
  ...
  if (options.projectionId !== undefined || options.trustedCompute !== undefined) {
    throw new SlopcameraCodeError("invalid-data", "A projection id or trusted-compute authority requires a registry projection source.");
  }
...
function parseCompiledWorkflowGraph(input) {
  ...
  if (parsedCompilationSha256 !== expected) {
    throw new SlopcameraCodeError("invalid-data", "Workflow compilation hash does not match its contents.", {
  ...
  if (canonicalJsonSha256(parsed.graph) !== canonicalJsonSha256(exactGraph)) {
    throw new SlopcameraCodeError("invalid-data", "Compiled workflow graph nodes are not normalized.");
  }
  const expectedGraphSha256 = workflowGraphHashFromNormalized(exactGraph);
  if (parsed.graphSha256 !== expectedGraphSha256) {
    throw new SlopcameraCodeError("invalid-data", "Workflow graph hash does not match its authenticated contents.", {
  ...
  if (recompiled.compilationSha256 !== workflowCompilationHashFromValidated(canonicalUnsigned)) {
    throw new SlopcameraCodeError("invalid-data", "Workflow compilation topology, requirements, or projection do not match the graph.");
  }
  return recompiled;
}
...
function consumeAuthoringValue(budget) {
  if (budget.consumed >= budget.maximum) {
    throw new SlopcameraCodeError("invalid-data", `Workflow authoring values exceed the ${String(MAX_AUTHORING_VALUES)} value limit.`);
  }
  budget.consumed += 1;
}
...
function encodeInputValue(input, state, dependencies, ancestors, depth, budget) {
  if (depth > MAX_AUTHORING_VALUE_DEPTH) {
    throw new SlopcameraCodeError("invalid-data", `Workflow input nesting exceeds ${String(MAX_AUTHORING_VALUE_DEPTH)} levels.`);
  }
  ...
  if (typeof input === "object" && input !== null && WORKFLOW_REF_BRAND in input) {
    throw new SlopcameraCodeError("invalid-data", "Use a typed Ref value created by this workflow graph builder.");
  }
  ...
  if (typeof input === "number") {
    if (!Number.isFinite(input)) {
      throw new SlopcameraCodeError("invalid-data", "Workflow node input numbers must be finite.");
    }
    return Object.is(input, -0) ? 0 : input;
  }
  if (typeof input !== "object") {
    throw new SlopcameraCodeError("invalid-data", `Workflow node input cannot contain ${typeof input} values.`);
  }
  if (ancestors.has(input)) {
    throw new SlopcameraCodeError("invalid-data", "Workflow node input cannot contain cycles.");
  }
  ancestors.add(input);
  try {
    if (Array.isArray(input)) {
      ...
      if (keys2.length !== length || keys2.some((key, index) => key !== String(index))) {
        throw new SlopcameraCodeError("invalid-data", "Workflow node input arrays must be dense and cannot have named properties.");
      }
      const encoded2 = [];
      for (let index = 0;index < length; index += 1) {
        const descriptor = descriptors2[String(index)];
        if (descriptor === undefined || descriptor.get !== undefined || descriptor.set !== undefined) {
          throw new SlopcameraCodeError("invalid-data", "Workflow node input arrays must contain plain data elements.");
        }
    ...
    if (!isPlainRecord(input)) {
      throw new SlopcameraCodeError("invalid-data", "Workflow node input accepts only JSON values and typed workflow references.");
    }
    if (Object.hasOwn(input, "$ref")) {
      throw new SlopcameraCodeError("invalid-data", "Use a typed Ref value instead of constructing the reserved $ref field.");
    }
    const descriptors = Object.getOwnPropertyDescriptors(input);
    ...
    for (const key of keys) {
      if (key === "__proto__") {
        throw new SlopcameraCodeError("invalid-data", "Workflow inputs cannot contain the reserved __proto__ object key.");
      }
      const descriptor = descriptors[key];
      if (descriptor === undefined || descriptor.get !== undefined || descriptor.set !== undefined) {
        throw new SlopcameraCodeError("invalid-data", "Workflow node input properties must be plain data properties.");
      }
...
function encodeControlDependencies(after, state) {
  ...
  const appendReference = (reference) => {
    if (!isOwnedRef(reference, state)) {
      throw new SlopcameraCodeError("invalid-data", "Operation control dependencies must be typed Ref values created by this workflow graph builder.");
    }
  ...
  if (length > MAX_SERIALIZED_NODE_DEPENDENCIES) {
    throw new SlopcameraCodeError("invalid-data", `Operation control dependencies cannot exceed ${String(MAX_SERIALIZED_NODE_DEPENDENCIES)} entries.`);
  }
  ...
  const keys = Object.keys(descriptors).filter((key) => descriptors[key]?.enumerable === true);
  if (keys.length !== length || keys.some((key, index) => key !== String(index))) {
    throw new SlopcameraCodeError("invalid-data", "Operation control dependencies must be a dense array without named properties.");
  }
  for (let index = 0;index < length; index += 1) {
    const descriptor = descriptors[String(index)];
    if (descriptor === undefined || descriptor.get !== undefined || descriptor.set !== undefined) {
      throw new SlopcameraCodeError("invalid-data", "Operation control dependencies must contain plain data elements.");
    }
...
function encodeOutputValue(output, state, ancestors, depth, budget) {
  if (depth > MAX_AUTHORING_VALUE_DEPTH) {
    throw new SlopcameraCodeError("invalid-data", `Workflow output nesting exceeds ${String(MAX_AUTHORING_VALUE_DEPTH)} levels.`);
  }
  ...
  if (typeof output !== "object" || output === null) {
    throw new SlopcameraCodeError("invalid-data", "Workflow outputs must contain only typed references, arrays, and named objects.");
  }
  ...
  try {
    if (Array.isArray(output)) {
      ...
      const keys2 = Object.keys(descriptors2).filter((key) => descriptors2[key]?.enumerable === true);
      if (keys2.length !== length || keys2.some((key, index) => key !== String(index))) {
        throw new SlopcameraCodeError("invalid-data", "Workflow output arrays must be dense and cannot have named properties.");
      }
      const encoded2 = [];
      for (let index = 0;index < length; index += 1) {
        const descriptor = descriptors2[String(index)];
        if (descriptor === undefined || descriptor.get !== undefined || descriptor.set !== undefined) {
          throw new SlopcameraCodeError("invalid-data", "Workflow output arrays must contain plain data elements.");
        }
    ...
    if (!isPlainRecord(output)) {
      throw new SlopcameraCodeError("invalid-data", "Workflow outputs accept only typed references, arrays, and plain objects.");
    }
    ...
    for (const key of keys) {
      if (key === "__proto__") {
        throw new SlopcameraCodeError("invalid-data", "Workflow outputs cannot contain the reserved __proto__ object key.");
      }
      const descriptor = descriptors[key];
      if (descriptor === undefined || descriptor.get !== undefined || descriptor.set !== undefined) {
        throw new SlopcameraCodeError("invalid-data", "Workflow output properties must be plain data properties.");
      }
...
function createReference(nodeKey, schemaId, state, path = []) {
  if (path.length > MAX_SERIALIZED_REF_PATH_SEGMENTS) {
    throw new SlopcameraCodeError("invalid-data", `Workflow reference paths cannot exceed ${String(MAX_SERIALIZED_REF_PATH_SEGMENTS)} segments.`);
  }
  ...
  const reference = Object.freeze({
    [WORKFLOW_REF_BRAND]: () => {
      throw new SlopcameraCodeError("internal", "A workflow reference type marker is not executable.");
    },
    at: (index) => {
      if (!Number.isSafeInteger(index) || index < 0) {
        throw new SlopcameraCodeError("invalid-data", "Workflow reference array indexes must be nonnegative safe integers.");
      }
      if (path.length >= MAX_SERIALIZED_REF_PATH_SEGMENTS) {
        throw new SlopcameraCodeError("invalid-data", `Workflow reference paths cannot exceed ${String(MAX_SERIALIZED_REF_PATH_SEGMENTS)} segments.`);
      }
    ...
    select: (key) => {
      if (typeof key !== "string" || key.length < 1 || key.length > 128) {
        throw new SlopcameraCodeError("invalid-data", "Workflow reference field names must contain 1\u2013128 characters.");
      }
      if (path.length >= MAX_SERIALIZED_REF_PATH_SEGMENTS) {
        throw new SlopcameraCodeError("invalid-data", `Workflow reference paths cannot exceed ${String(MAX_SERIALIZED_REF_PATH_SEGMENTS)} segments.`);
      }
...
function createState(provider) {
  ...
  for (const input of providerDiscovery(provider)) {
    ...
    if (discovery.has(key)) {
      throw new SlopcameraCodeError("conflict", `Duplicate operation discovery entry: ${key}`, { kind: item.kind, version: item.version });
    }
...
function defineWorkflowFragment(build) {
  if (typeof build !== "function") {
    throw new SlopcameraCodeError("invalid-data", "Workflow fragments require a build function.");
  }
...
function operationContract(provider, kind, version) {
  ...
  if (discovery === undefined) {
    throw new SlopcameraCodeError("unsupported-plan", `Unsupported operation: ${kind}@${String(version)}`, { kind, version });
  }
...
class WorkflowGraphBuilder {
  ...
  operationByKind(keyInput, request, options = {}) {
    ...
    if (discovery === undefined) {
      throw new SlopcameraCodeError("unsupported-plan", `Unsupported operation: ${request.kind}@${String(request.version)}`, { kind: request.kind, version: request.version });
    }
    return this.#addOperation(keyInput, {
      input: request.input,
      inputSchemaId: discovery.inputSchemaId,
  ...
  compute(keyInput, definition, input, options = {}) {
    if (typeof definition !== "object" || definition === null) {
      throw new SlopcameraCodeError("invalid-data", "Compute nodes require a definition created by defineCompute().");
    }
    ...
    if (existing !== undefined && existing !== normalizedDefinition) {
      throw new SlopcameraCodeError("conflict", `Duplicate trusted compute key: ${compute.key}`, { key: compute.key });
    }
  ...
  #nodeKey(keyInput) {
    ...
    if (this.#state.nodes.has(key)) {
      throw new SlopcameraCodeError("conflict", `Duplicate workflow node key: ${key}`, { nodeKey: key });
    }
    if (this.#state.nodes.size >= MAX_SERIALIZED_GRAPH_NODES) {
      throw new SlopcameraCodeError("invalid-data", `Workflow nodes cannot exceed ${String(MAX_SERIALIZED_GRAPH_NODES)} entries.`);
    }
    return key;
  }
  #addOperation(keyInput, request, options) {
    ...
    if (request.inputSchemaId !== discovery.inputSchemaId || request.outputSchemaId !== discovery.outputSchemaId) {
      throw new SlopcameraCodeError("invalid-data", `Operation contract schema mismatch for ${request.kind}@${String(request.version)}.`, {
...
class PortableWorkflowBuilder {
  ...
  constructor(builder) {
    ...
    this.diagram = Object.freeze({
      check: (key, input, options = {}) => this.#builder.operation(key, PORTABLE_SLOPCAMERA_OPERATION_CONTRACTS["slopcamera.diagram.check"], input, options),
      render: (key, input, options = {}) => this.#builder.operation(key, PORTABLE_SLOPCAMERA_OPERATION_CONTRACTS["slopcamera.diagram.render"], input, options)
    });
    this.image = Object.freeze({
      generate: (key, input, options = {}) => this.#builder.operation(key, PORTABLE_SLOPCAMERA_OPERATION_CONTRACTS["slopcamera.image.generate"], input, options),
      vectorize: (key, input, options = {}) => this.#builder.operation(key, PORTABLE_SLOPCAMERA_OPERATION_CONTRACTS["slopcamera.image.vectorize"], input, options)
    });
  }
  static create() {
...
function workflowDefinitionIdentity(options) {
  if (typeof options.build !== "function") {
    throw new SlopcameraCodeError("invalid-data", "Workflow definitions require a build function.");
  }
  ...
  if (!Number.isSafeInteger(options.version) || options.version < 1) {
    throw new SlopcameraCodeError("invalid-data", "Workflow versions must be positive safe integers.");
  }
  return { id, inputSchemaId, version: options.version };
}
function assertOptionsObject(value, name) {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new SlopcameraCodeError("invalid-data", `${name} must be an object.`);
  }
}
function assertSchemaCapability(value, name) {
  if (typeof value !== "object" || value === null || typeof value.safeParse !== "function") {
    throw new SlopcameraCodeError("invalid-data", `${name} must provide a synchronous safeParse function.`);
  }
}
...
function defineCompute(options) {
  ...
  if (typeof options.run !== "function") {
    throw new SlopcameraCodeError("invalid-data", "Trusted compute definitions require a run function.");
  }
  ...
  if (identity.bounds.maxDurationMs > MAX_TRUSTED_COMPUTE_DURATION_MS) {
    throw new SlopcameraCodeError("invalid-data", "Trusted compute duration exceeds the host maximum.");
  }
  return Object.freeze({
    [TRUSTED_COMPUTE_BRAND]: true,
...
function seconds(value) {
  if (!Number.isFinite(value) || value < 0) {
    throw new SlopcameraCodeError("invalid-data", "Seconds must be a finite nonnegative number.");
  }
  const microseconds = value * 1e6;
  if (!Number.isSafeInteger(microseconds)) {
    throw new SlopcameraCodeError("invalid-data", "Seconds must resolve to an integer number of safe microseconds.");
  }
  return microseconds;
}

// src/code/runtime.ts
var WORKFLOW_NODE_RECEIPT_VERSION = "slopcamera-workflow-node-receipt-v1";
var WORKFLOW_NODE_RECEIPT_HASH_DOMAIN = "slopcamera.workflow.node-receipt/v1";
var MAX_WORKFLOW_RESULT_BYTES = 96 * 1024 * 1024;
var MAX_WORKFLOW_RESULT_DEPTH = 320;
var MAX_WORKFLOW_RESULT_VALUES = 1300000;
function createSlopcameraCodeHost(options) {
  if (typeof options !== "object" || options === null) {
    throw new SlopcameraCodeError("invalid-data", "An Slopcamera Code host must be an object.");
  }
  if (typeof options.execute !== "function") {
    throw new SlopcameraCodeError("invalid-data", "An Slopcamera Code host requires an execute function.");
  }
  if (options.admit !== undefined && typeof options.admit !== "function") {
    throw new SlopcameraCodeError("invalid-data", "An Slopcamera Code host admit value must be a function when provided.");
  }
  return Object.freeze({
...
class SlopcameraWorkflowRunError extends SlopcameraCodeError {
  completedReceipts;
  failedNode;
  runCause;
...
function projectedValue(reference, values) {
  ...
  if (current === undefined) {
    throw new SlopcameraCodeError("internal", `Workflow reference producer ${reference.$ref.nodeKey} has not completed.`, { nodeKey: reference.$ref.nodeKey });
  }
  for (const segment of reference.$ref.path ?? []) {
    if (typeof segment === "number") {
      if (!Array.isArray(current) || segment >= current.length) {
        throw new SlopcameraCodeError("invalid-data", `Workflow reference ${reference.$ref.nodeKey} has an invalid array projection.`, { nodeKey: reference.$ref.nodeKey, segment });
      }
      ...
      if (typeof current !== "object" || current === null || Array.isArray(current) || !Object.hasOwn(current, segment)) {
        throw new SlopcameraCodeError("invalid-data", `Workflow reference ${reference.$ref.nodeKey} has an invalid object projection.`, { nodeKey: reference.$ref.nodeKey, segment });
      }
      current = current[segment];
    }
    if (current === undefined) {
      throw new SlopcameraCodeError("invalid-data", `Workflow reference ${reference.$ref.nodeKey} projected an undefined value.`, { nodeKey: reference.$ref.nodeKey, segment });
    }
  }
  return current;
}
function resolveValue(value, values) {
  ...
  if (typeof value === "object" && value !== null) {
    ...
    for (const key of Object.keys(record).sort()) {
      ...
      if (item === undefined) {
        throw new SlopcameraCodeError("internal", `Compiled workflow value ${key} is undefined.`);
      }
...
function publicOperationNode(node) {
  if (isComputeGraphNode(node)) {
    throw new SlopcameraCodeError("unsupported-plan", `The public projection does not support trusted compute at node ${node.key}.`, {
      executorKind: "compute",
      nodeKey: node.key,
  ...
  if (!isOperationGraphNode(node)) {
    throw new SlopcameraCodeError("unsupported-plan", `The public projection does not support the executor at node ${node.key}.`, { nodeKey: node.key, projectionId: PUBLIC_WORKFLOW_REGISTRY_PROJECTION.id });
  }
  const operation = node.executor.operation;
  if (operation.version !== 2 || !isPortableSlopcameraOperationKind(operation.kind)) {
    throw new SlopcameraCodeError("unsupported-plan", `Unsupported operation: ${operation.kind}@${String(operation.version)}`, {
      kind: operation.kind,
      nodeKey: node.key,
      projectionId: PUBLIC_WORKFLOW_REGISTRY_PROJECTION.id,
...
function workflowNodeFailure(error, node, completedReceipts) {
  ...
  return new SlopcameraWorkflowRunError(code, `Workflow node ${node.key} (${node.executor.operation.kind}@2) failed: ` + slopcameraCodeErrorMessage(error), {
    cause: error,
    completedReceipts,
    failedNode: {
...
async function runBuiltWorkflow(built, options) {
  ...
  for (const wave of compilation.topologicalWaves) {
    const outcomes = await Promise.all(wave.map(async (nodeKey) => {
      ...
      if (node === undefined) {
        throw new SlopcameraCodeError("internal", `Compiled workflow topology lost node ${nodeKey}.`, { nodeKey });
      }
      publicOperationNode(node);
      try {
        return {
    ...
    if (context.signal.aborted) {
      const cause = new SlopcameraCodeError("cancelled", "Workflow execution was cancelled.");
  ...
  try {
    ...
    throw workflowRunFailure(error, `Workflow output resolution failed: ${slopcameraCodeErrorMessage(error)}`, receipts);
  }
  return Object.freeze({
    compilation,
    output,
