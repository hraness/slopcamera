...
function defaultCacheDirectory() {
  ...
  if (explicit !== undefined && explicit.trim() !== "")
    return explicit;
  if (process.platform === "win32") {
    return join2(process.env.LOCALAPPDATA ?? homedir(), "slopcamera");
  }
  if (process.platform === "darwin")
    return join2(homedir(), "Library", "Caches", "slopcamera");
  return join2(process.env.XDG_CACHE_HOME ?? join2(homedir(), ".cache"), "slopcamera");
}
...
async function runVectorizeWorker(input, options) {
  ...
  const temporaryRoot = await mkdtemp2(join3(tmpdir(), "slopcamera-vectorize-"));
  let result;
  try {
