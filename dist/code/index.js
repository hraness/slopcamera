...
function scatter(input) {
  ...
  return deepFreezeJson(parseSpatialValue(SpatialEntitySchema, {
    ...
    instances: positions.map(([x, , z3]) => ({ position: [x, 0, z3], rotation: [0, 0, 0, 1], scale: [1, 1, 1] }))
...
var moduleResultSchema = z3.strictObject({
  entities: z3.array(z3.unknown()).max(SPATIAL_SCENE_LIMITS.entities),
