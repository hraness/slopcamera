...
var scalarKeySchema = z2.strictObject({ timeUs: SpatialTimeUsSchema, value: z2.number().finite().min(0).max(1) });
var vec3KeySchema = z2.strictObject({ timeUs: SpatialTimeUsSchema, value: SpatialVec3Schema });
var quaternionKeySchema = z2.strictObject({ timeUs: SpatialTimeUsSchema, value: SpatialQuaternionSchema });
...
function grid(input) {
  ...
  return deepFreezeJson(parseSpatialValue(z2.array(SpatialVec3Schema), Array.from({ length: input.rows * input.columns }, (_, cell) => [origin[0] + cell % input.columns * sx, origin[1], origin[2] + Math.floor(cell / input.columns) * sz]), "grid"));
}
...
function scatter(input) {
  ...
  for (let placed = 0;placed < input.count; placed++) {
    ...
    for (let attempt = 0;attempt < SCATTER_MAX_ATTEMPTS && !done; attempt++) {
      ...
      const z3 = region.minZ + random() * (region.maxZ - region.minZ);
      if (spacing === 0 || accepted.every(([px, , pz]) => (px - x) * (px - x) + (pz - z3) * (pz - z3) >= spacing * spacing)) {
        accepted.push([x, 0, z3]);
        done = true;
      }
    }
    if (!done)
  ...
  return deepFreezeJson(parseSpatialValue(z2.array(SpatialVec3Schema), accepted, "scatter"));
}
...
var SpatialSolveAnchorKeySchema = z3.string().min(1).max(128).regex(/^[a-zA-Z0-9][a-zA-Z0-9_-]*$/u, "Solve keys start alphanumeric and may contain _ or -.");
var axisSchema = z3.enum(["x", "y", "z"]);
var boundedNumber = z3.number().finite().min(-1e6).max(1e6);
var positiveDimension = z3.number().finite().min(0.000001).max(1e6);
var scaleTuple = z3.tuple([positiveDimension, positiveDimension, positiveDimension]);
var SpatialSolveBoundsSchema = z3.strictObject({ min: SpatialVec3Schema, max: SpatialVec3Schema }).refine((bounds2) => bounds2.min.every((value, index) => value <= bounds2.max[index]), "Bounds min must not exceed max.");
var SpatialSolveRelationSchema = z3.discriminatedUnion("kind", [
  z3.strictObject({ kind: z3.literal("onTopOf"), target: SpatialSolveAnchorKeySchema }),
  z3.strictObject({
    kind: z3.literal("nextTo"),
    target: SpatialSolveAnchorKeySchema,
    axis: axisSchema.optional(),
    side: z3.enum(["before", "after"]).optional(),
    gap: boundedNumber.optional()
  }),
  z3.strictObject({ kind: z3.literal("facing"), target: SpatialSolveAnchorKeySchema, up: SpatialVec3Schema.optional() }),
  z3.strictObject({
    kind: z3.literal("align"),
    target: SpatialSolveAnchorKeySchema,
    axis: axisSchema,
    edge: z3.enum(["min", "center", "max"]).optional()
  }),
  z3.strictObject({
    kind: z3.literal("at"),
  ...
  z3.strictObject({ kind: z3.literal("groundSnap"), floorY: boundedNumber.optional() })
]);
var SpatialSolveGoalSchema = z3.strictObject({
  entityKey: SpatialSolveEntityKeySchema,
  relations: z3.array(SpatialSolveRelationSchema).min(1).max(SPATIAL_SOLVE_LIMITS.relationsPerGoal)
});
var SpatialSolveBaseSchema = z3.strictObject({
  transform: SpatialTransformSchema,
  bounds: SpatialSolveBoundsSchema.optional()
});
var SpatialSolveRequestSchema = z3.strictObject({
  goals: z3.array(SpatialSolveGoalSchema).min(1).max(SPATIAL_SOLVE_LIMITS.goals),
  bases: z3.record(SpatialSolveAnchorKeySchema, SpatialSolveBaseSchema)
});
var SpatialSolveBasePatchSchema = z3.strictObject({
  transform: SpatialTransformSchema.optional(),
  bounds: SpatialSolveBoundsSchema.optional()
});
var SpatialSolveGoalsFileSchema = z3.strictObject({
  goals: z3.array(SpatialSolveGoalSchema).min(1).max(SPATIAL_SOLVE_LIMITS.goals),
  bases: z3.record(SpatialSolveAnchorKeySchema, SpatialSolveBasePatchSchema).optional()
});
